...
use foundry_compilers_artifacts_solc::{
    CompactContract, CompactContractBytecode, CompactContractBytecodeCow, CompactContractRef,
    DevDoc, StorageLayout, UserDoc,
};
...
impl From<Contract> for CompactContract {
    fn from(c: Contract) -> Self {
        ...
        Self {
            bin: bytecode.as_ref().map(|b| b.object.clone()),
            bin_runtime: bytecode.as_ref().map(|b| b.object.clone()),
            abi: c.abi,
        }
    }
}
